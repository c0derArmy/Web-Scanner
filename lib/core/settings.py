...
# clone link
CLONE = "https://github.com/c0derArmy/web-scanner.git"

# issue link
ISSUE_LINK = "https://github.com/c0derArmy/web-scanner/issues"
...
DISCLAIMER = (
    "[!] legal disclaimer: Usage of Web-Scanner for attacking targets without prior mutual consent is illegal. "
...
\033[1;31m{}
\033[0m""".format(CLONE, SAYING, DISCLAIMER)
...
DEFAULT_USER_AGENT = "Web-Scanner/{} (Language=Python/{}; Platform={})".format(
...
def find_application(application, opt="path"):
    ...
    conf_parser.read(io.BytesIO(read_conf.encode('utf-8')))
...
def get_browser_version(output=True):
    ...
    try:
        ...
        output = subprocess.check_output(firefox_version_command, text=True)
    except OSError:
...
def get_md5sum(url="https://raw.githubusercontent.com/Ekultek/Zeus-Scanner/master/etc/checksum/md5sum.md5"):
    ...
    """
    # Disabled checksum check for Python 3 conversion
    return True
