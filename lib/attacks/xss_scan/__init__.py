...
def create_urls(url, payload_list, tamper=None, verbose=False, force=False):
    ...
    with tf as tmp:
        for i, payload in enumerate(payload_list):
            ...
            tmp.write(loaded_url.encode('utf-8'))
    return tf_name
...
def scan_xss(url, agent=None, proxy=None):
    ...
    try:
        ...
        html_data_str = html_data.decode('utf-8', errors='ignore') if isinstance(html_data, bytes) else html_data
        for db in lib.core.settings.DBMS_ERRORS.keys():
            for item in lib.core.settings.DBMS_ERRORS[db]:
                if re.findall(item, html_data_str):
                    return "sqli", db
        if status != 404:
            if query in html_data_str:
                return True, None
        return False, None
